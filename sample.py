    ...
    if opt['phase'] == 'train':
        while current_step < n_iter:
            ...
            for _, train_data in enumerate(train_loader):
                ...
                # validation
                if current_step % (opt['train']['val_freq'] * 4) == 0:
                    ...
                    for idx in range(sample_sum):
                        ...
                        sample_img = Metrics.tensor2img(
                            visuals['SAM'], out_type=np.uint16, min_max=(-1, 1))  # uint8
                        # generation
                        Metrics.save_img(
        ...
        for idx in range(sample_sum):
            ...
            if show_img_mode == 'single':
                ...
                for iter in range(0, sample_num):
                    Metrics.save_img(
                        Metrics.tensor2img(sample_img[iter], out_type=np.uint16, min_max=(-1, 1)), '{}/{}_{}_sample_{}.png'.format(result_path, current_step, idx, iter))
            else:
                # grid img
                sample_img = Metrics.tensor2img(visuals['SAM'], out_type=np.uint16, min_max=(-1, 1)) 
                Metrics.save_img(
                    sample_img, '{}/{}_{}_sample_process.png'.format(result_path, current_step, idx))
                Metrics.save_img(
                    Metrics.tensor2img(visuals['SAM'][-1], out_type=np.uint16, min_max=(-1, 1)), '{}/{}_{}_sample.png'.format(result_path, current_step, idx))
            
            sample_imgs.append(Metrics.tensor2img(visuals['SAM'][-1], out_type=np.uint16, min_max=(-1, 1)))
